...
export async function generatePdf({ summaryMarkdown, companyName, ticker, quarter, eventDate, outputPath }) {
  ...
  // Launch headless Chrome — may not be available in constrained environments
  let browser;
  try {
    ...
    throw new Error(
      `Chrome not available (PDF skipped): ${launchErr.message}. ` +
      "Emails will still send without PDF attachment."
    );
  }
